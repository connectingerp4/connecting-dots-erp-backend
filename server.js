...
app.get('/api/leads', authMiddleware, requireRole(['SuperAdmin', 'Admin', 'EditMode', 'ViewMode']), async (req, res) => {
  try {
    const { populate, ignoreDisplayLimit } = req.query;
    let filter = {};
    ...
    const shouldIgnoreDisplayLimit = ignoreDisplayLimit === 'true';
    ...
    if (!shouldIgnoreDisplayLimit && maxLeadsToDisplay > 0) {
...
app.get('/api/analytics', authMiddleware, requireRole(['SuperAdmin', 'Admin']), async (req, res) => {
  try {
    ...
    const dateMatch = Object.keys(createdAtFilter).length
      ? { createdAt: createdAtFilter }
      : {};

    // Count total leads
    const totalLeads = await User.countDocuments(dateMatch);
    ...
    const allLeads = await User.find(dateMatch, { createdAt: 1 }).lean();
